...
import { ArrowUpDown, Trophy, Medal, UserIcon, HelpCircle, ChevronDown } from "lucide-react"
...
import { getLeaderboard, getCurrentUserStats } from "@/app/actions/leaderboard-actions"
// Define LeaderboardEntry type locally
...
export function LeaderboardTable() {
  ...
  const [sortKey, setSortKey] = useState<SortKey>("earnings") // Default to sorting by earnings
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc") // Default to highest values first
  ...
  useEffect(() => {
    async function fetchLeaderboardData() {
      ...
      try {
        ...
        const response = await getLeaderboard(displayLimit);
  ...
  // Manual sorting after data is fetched
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="rounded-md border">
            ...
            {/* Load More Button */}
            {sortedUsers.length > 0 && sortedUsers.length < totalEntries && (
            ...
            {userId && currentUserStats && !sortedUsers.some(u => userId && u.userId === userId) && (
